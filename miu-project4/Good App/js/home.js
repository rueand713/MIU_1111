...
function getCheckBoxValue() {
	...
	// creates the news stream pattern and orders the items from newest to oldest
	function getNewsStream() {
		...
		for (var i=0; i < localStorage.length; i++) {
			var key = localStorage.key(i),
				...
				objName = newObj.name[1];
				
			// stores the date in parsed and unparsed form along 
			// with the routine that is associated with it
			// for comparison and identification in the get data function
				values2 = [coreVal, startVal, objName];
				values.push(values2);
		...
		for (var y=0; y<values.length; y++) { 
			...
			// compares the value of newVal with the value associated
			// with the current index of values array
			// because of the need to pass along the complete array I am using
			// if conditional to determine the greater or lesser value rather than
			// the Math method of .max
		   for (var x=0; x<values.length; x++) {
				if (newVal < values[x][1]) {
					newVal = values[x];
				} else {
					newVal = nVal[y];
				}
